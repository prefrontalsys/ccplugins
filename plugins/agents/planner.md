...
- Every step names specific files, functions, or locations (no "update the config" vagueness)
- Constraints section is non-empty and reflects actual codebase exploration
- Risks section is non-empty
- Confidence is ≥80, or the plan explicitly flags what's blocking higher confidence
- A worker agent could execute any step without asking clarifying questions about WHICH file, function, or location to touch. Steps may still involve judgment on HOW — the plan is not required to pre-empt every decision, only to eliminate ambiguity of scope.
