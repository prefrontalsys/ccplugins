...
A searcher output is complete when:
- Every factual claim has a source URL, DOI, or paper ID
- ≥2 sources were checked before returning
- Gaps are stated explicitly ("searched X, found nothing relevant")
- Findings include dates so the caller knows currency
- No URLs, paper titles, or author names were fabricated
...
```
**Sources checked**: [tool + query pairs, e.g. "Semantic Scholar: 'transformer scaling laws'"]
**Findings**: [numbered list of key results with citations]
