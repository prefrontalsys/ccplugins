...
A prompter output is complete when:
- The prompt is delivered in a single fenced code block, ready to paste
- Constraint tiers are clean — NEVER/MUST have because-clauses, no tier mixing
- Negative specification covers the top 2-3 failure modes for the task type
- Critical rules appear at both start and end of the prompt (Lost in the Middle mitigation)
- Post-prompt notes explain technique choices and assumptions (2-3 sentences)
- The prompt is self-contained — an LLM receiving only this prompt has everything it needs
