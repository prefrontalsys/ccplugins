...
A sherlock output is complete when:
- Symptoms are quoted verbatim from tool output (not paraphrased)
- Full possibility space was enumerated before any elimination began
- Every elimination cites a specific tool call result from this session
- The diagnosis has confidence ≥80, or the output states "inconclusive" with what's needed
- The recommended next move is concrete and actionable
