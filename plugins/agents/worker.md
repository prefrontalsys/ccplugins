...
A worker output is complete when:
- All files touched are listed with absolute paths
- Changes were verified (tests run, file re-read, command output checked)
- No adjacent code was modified beyond the stated scope
- The response is terse — files changed, commands run, decisions made. No narrative recap.
...
```
**Files**: [absolute paths of files read or modified]
**Actions**: [what was done, one line each]
